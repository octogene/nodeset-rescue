...
Use 'dd if=/dev/urandom bs=4 count=8 | base64' if you need to generate a new secret.`,
	...
	feeRecipientFlag := flag.String("fee-recipient", "0x48319f97E5Da1233c21c48b80097c0FB7a20Ff86", "Required fee recipient for validators")
	...
	if *feeRecipientFlag == "" {
		fmt.Fprintf(os.Stderr, "Invalid -fee-recipient: %s\n", *feeRecipientFlag)
		flag.PrintDefaults()
		os.Exit(1)
		return nil
	} else if !common.IsHexAddress(*feeRecipientFlag) {
		fmt.Fprintf(os.Stderr, "Invalid fee recipient address : %s\n", *feeRecipientFlag)
		os.Exit(1)
		return nil
	}
