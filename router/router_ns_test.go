...
func TestRouterPBPSWCheater(t *testing.T) {
	errs := make(chan error)
	rt := setup(t, errs)
	...
	var errorResponse = fmt.Sprintf("validator is stakewise, but fee recipient doesn't match the mevEscrow contract address at %s", fr.String())
	if !strings.EqualFold(eMap["error"], errorResponse) {
		t.Fatal("unexpected response", string(body))
	}
...
func TestRouterRVSW(t *testing.T) {
	errs := make(chan error)
	rt := setup(t, errs)
...
func TestRouterRVSWCheater(t *testing.T) {
	errs := make(chan error)
	rt := setup(t, errs)
	...
	if !found {
		t.Fatalf("couldn't find validator in mock with 0x01 credential set to %x with %s", vault[:], fr.String())
	}
