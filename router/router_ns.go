...
func (pr *ProxyRouter) prepareBeaconProposerGuard(proposers gbp.PrepareBeaconProposerRequest, ctx context.Context) (gbp.AuthenticationStatus, error) {
	...
	for _, proposer := range proposers {
		...
		if authStatus, err := pr.stakewisePBPGuard(authInfo, validatorInfo, proposer.FeeRecipient); authStatus != gbp.Allowed {
			return authStatus, err
		}
	}

	// Every check passed, record usage
	recordErr := pr.ut.RecordUsage(indices)
	if recordErr != nil {
		pr.Logger.Error("Error while recording validator usage", zap.Error(recordErr))
	}
...
func (pr *ProxyRouter) registerValidatorGuard(validators gbp.RegisterValidatorRequest, ctx context.Context) (gbp.AuthenticationStatus, error) {
	...
	for _, validator := range validators {
		...
		_, err := rptypes.HexToValidatorPubkey(pubkeyStr)
		if err != nil {
		...
		if strings.EqualFold(pr.ExpectedFeeRecipient, validator.Message.FeeRecipient) {
		...
		pr.Logger.Warn("register_validator called with unexpected fee recipient",
			zap.String("expected", pr.ExpectedFeeRecipient),
...
func (pr *ProxyRouter) Init() {
	...
	pr.ut = NewSQLiteUsageTracker(pr.Logger)
}
